...
function verify_validate_zip {
    ...
    pyflakes lambda.py
    aws cloudformation validate-template --template-body file://walkme-videos-notification.cfn.yaml 2>&1 > /dev/null
...
function main {
    ...
    local -r BUCKET_PATH="cf-templates-oregon-bp/walkme-videos-notification/"
    ...
    for file in code-${BITBUCKET_COMMIT}.zip walkme-videos-notification.cfn.yaml
    do
