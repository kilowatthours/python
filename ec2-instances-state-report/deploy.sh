...
function verify_validate_zip {
    ...
    pyflakes main.py
    aws cloudformation validate-template --template-body file://ec2-instances-state-report.cfn.yaml 2>&1 > /dev/null
...
function main {
    ...
    local -r BUCKET_PATH="cf-templates-oregon-bp/ec2-instances-state-report/"
    ...
    for file in code-${BITBUCKET_COMMIT}.zip ec2-instances-state-report.cfn.yaml
    do
